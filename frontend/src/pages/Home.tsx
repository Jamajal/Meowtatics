...
export default function Home(){
    ...
    const getCats = async () => {
        const catsToRender = await api.loadCats()
        
        if(catsToRender)
            setCats(catsToRender);
    }
    ...
    return(
        <div>
            ...
            { !auth.user && <Link to='/signin'>Login</Link> }
            { auth.user && <Link to='/' onClick={handleLogout}>Logout</Link> }
            <div className=" w-screen p-10 flex flex-col items-center">
                <div className="w-full">
                    ...
                    />
                    <button className="teste px-4 py-1 text-base font-medium text-white rounded bg-orange-150"
                        onClick={() => {
                            console.log(cats)
                        }}
                    >Filtrar</button>
